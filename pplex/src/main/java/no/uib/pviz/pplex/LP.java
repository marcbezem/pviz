...
import no.uib.pviz.util.Either;
import no.uib.pviz.util.Left;
import no.uib.pviz.util.Right;
...
public class LP {
    ...
    public static FieldMatrix<BigFraction> addBlock(
        ...
        String error = String.format("Illegal operation: Cannot add a matrix block"
      ...
      return new Array2DRowFieldMatrix<>(cdata);
    }
     ...
     */
    private Either<String,Integer> entering(boolean dual) {
        String error1 = "Incumbent basic solution is optimal.";
        String error2 = String.format("Incumbent basic solution is %s infeasible",
                                   dual ? "dually" : "primal");

        if (optimal(dual))   return new Left<>(error1);
        if (!feasible(dual)) return new Left<>(error2);
     ...
     */
    private Either<String,Integer> leaving(int entering, boolean dual) {
        ...
        String error = "Program is unbounded.";
        if (unbounded) return new Left<>(error);
        ...
        for (int i = 0; i < sd.getDimension(); i++) {
            ...
            if (!denom.equals(BigFraction.ZERO)) {
                ...
                if (num.compareTo(BigFraction.ZERO) > 0) return new Right<>(i);
            }
        }
     ...
     *         A linear program after one iteration.
     */
    public Either<String,LP> pivot(boolean dual) {
        Either<String,Integer> e = entering(dual);
        int entering = 0;
        int leaving  = 0;
        switch (e.type()) {
          case LEFT: return new Left<>(e.getLeft());
          case RIGHT:
            entering = e.getRight();
            Either<String,Integer> l = leaving(entering,dual);
            switch (l.type()) {
              case LEFT:  return new Left<>(e.getLeft());
              case RIGHT:
                leaving = l.getRight();
        ...
        LP result = dual?pivot(leaving, entering)
